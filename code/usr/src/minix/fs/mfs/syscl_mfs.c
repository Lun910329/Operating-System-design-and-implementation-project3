...
// syscl fix here
//
static struct super_block syscl_super; // syscl super_block
static struct super_block *sb = &syscl_super;   /* super block */
...
int fs_inodewalker()
{
	...
	type = IMAP;
	//sb = get_super(fs_dev);
	sb->s_dev = fs_dev;	/* read_super() needs to know which dev */
	read_super(sb);
	lsuper();
	init_global();
